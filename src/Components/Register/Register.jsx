...
export default function Register() {
    ...
    function ValidateRegisterForm(){
        let schema=Joi.object({
            ...
            email:Joi.string().email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }).pattern(new RegExp('^[A-Z](\.?[a-z0-9]){5,}@g(oogle)?mail\.com$')).message("email must contain at least 6 character and first litter character capital").required(),
            password:Joi.string().pattern(/^[A-Z][a-z]{6}/).message('passwords must at least 6 character and contain first litter capital').required(),
        })
    ...
    return (
        <div className=' m-auto LoginAndSignUpStyle my-4  p-3 rounded-3' >
            <h3 className='text-center w-100 text-info  fw-bold'>Register</h3>
        ...
        <form className='my-2' onSubmit={submitData} autoComplete="off">
            ...
            <input onChange={getUserData}   autoComplete="new-password"    type="text" name="first_name" id="first_name" className='form-control my-input mb-3' placeholder='Enter First Name .....'/>
            <div className=''>{alerm}</div>
            <label htmlFor='last_name'>last Name :</label>
            <input onChange={getUserData}  autoComplete="new-password"   type="text" name="last_name" id="last_name" className='form-control my-input mb-3' placeholder='Enter Last Name .....'/>
